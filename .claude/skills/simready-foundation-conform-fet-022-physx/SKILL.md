---
name: simready-foundation-conform-fet-022-physx
description: "Use for repairing exact FET_022_PHYSX SimReady conformance for PhysX driven-joint conformance. Use when a profile, validation report, or user request names FET_022_PHYSX; default to version `0.2.0` unless a profile or report pins another version."
license: Apache-2.0
metadata:
...
# SimReady Conform FET_022_PHYSX

## Purpose

Use this exact feature skill when the selected profile, validation report, or user request names `FET_022_PHYSX`. It repairs or stages PhysX driven-joint conformance without drifting into another runtime contract.

Default to `FET_022_PHYSX@0.2.0` when the user asks for this feature without a version. Use an older integer version only when the profile, validation report, or user explicitly pins it. If the report names a different `FET_###_RUNTIME` feature, switch to that feature's matching skill before editing.

## Source of Truth

Before changing an asset or package, read:

- `nv_core/tiers/simready_foundation_tier_core/simready/foundation/tier_core/features/FET_022_PHYSX-0.1.0.json`
- `nv_core/tiers/simready_foundation_tier_core/simready/foundation/tier_core/features/FET_022_PHYSX-0.2.0.json`
...
2. Load the selected `FET_022_PHYSX` manifest and the feature markdown before editing.
...
5. Repair only the requirements listed by the selected `FET_022_PHYSX` manifest and its dependencies.
...
7. Summarize the selected `FET_022_PHYSX` version, changed files, validation evidence, and the first remaining blocker or next exact feature gate.

## Feature Guidance

- Repair only the requirements listed by the selected `FET_022_PHYSX` manifest and its dependencies.
...
| Field | Meaning |
|---|---|
| `feature` | Exact feature ID and version, for example `FET_022_PHYSX@0.2.0`. |
