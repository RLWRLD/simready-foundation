...
class InterfaceConnectionRule(RuleInterface):
    ...
    def get_configuration_parameters(self) -> list[RuleConfigurationParam]:
        ...
        return [
            ...
            RuleConfigurationParam(
                name="excluded_variant_sets",
                display_name="Excluded Variant Sets",
                param_type=list,
                description="Payload subfolders that must not become interface variant sets.",
                default_value=[],
            ),
            RuleConfigurationParam(
                name="include_none_variant",
                display_name="Include None Variant",
                param_type=bool,
                description="Add an empty 'none' option to generated variant sets.",
                default_value=True,
            ),
            RuleConfigurationParam(
                name="capitalize_variant_names",
                display_name="Capitalize Variant Names",
                param_type=bool,
                description="Capitalize generated variant names such as Enabled and Disabled.",
                default_value=False,
            ),
        ]
    ...
    def process_rule(self) -> str | None:
        ...
        excluded_variant_sets = set(params.get("excluded_variant_sets") or [])
        include_none_variant = params.get("include_none_variant", True)
        capitalize_variant_names = params.get("capitalize_variant_names", False)
    ...
    def _generate_folder_variants(
        ...
        excluded_variant_sets: set[str],
        include_none_variant: bool,
        capitalize_variant_names: bool,
    ) -> None:
        ...
        Args:
            ...
            excluded_variant_sets: Folder-derived variant sets to skip.
            include_none_variant: Whether to add an empty ``none`` option.
            capitalize_variant_names: Whether to capitalize generated option names.
        ...
        for entry in os.listdir(payloads_abs_path):
            ...
            if variant_set_name in excluded_variant_sets:
                self.log_operation(f"Skipped excluded variant set '{variant_set_name}'")
                continue
            ...
            # Add the optional empty variant first.
            if include_none_variant and _NONE_VARIANT_NAME not in variant_set_spec.variants:
                Sdf.VariantSpec(variant_set_spec, _NONE_VARIANT_NAME)
            ...
            for asset_file in variant_assets:
                ...
                if capitalize_variant_names:
                    variant_name = variant_name[:1].upper() + variant_name[1:]
            ...
            if variant_set_name in default_variant_selections:
                ...
                default_selection = _NONE_VARIANT_NAME if include_none_variant else None
            ...
            empty_suffix = " + none" if include_none_variant else ""
            self.log_operation(
                f"Created variant set '{variant_set_name}' with {len(variant_assets)} variants{empty_suffix}"
            )
