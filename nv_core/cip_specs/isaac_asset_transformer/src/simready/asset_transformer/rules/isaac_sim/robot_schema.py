...
try:  # Kit-extension robot schema; optional so the rule imports without Kit.
    import usd.schema.isaac.robot_schema as rs
    from usd.schema.isaac.robot_schema import utils as robot_schema_utils

    _HAS_ISAAC_SCHEMA = True
except Exception:  # pragma: no cover - exercised only in Kit runtimes
    rs = None
    robot_schema_utils = None
    _HAS_ISAAC_SCHEMA = False

from .. import utils

# Applied-schema token and relationship names authored by the Kit-free fallback.
# These match ``usd.schema.isaac.robot_schema`` and the SimReady RC.007 validator
# (``capabilities/isaac_sim/robot_core/validation.py``).
_ROBOT_API_SCHEMA_NAME = "IsaacRobotAPI"
_ROBOT_LINKS_REL_NAME = "isaac:physics:robotLinks"
_ROBOT_JOINTS_REL_NAME = "isaac:physics:robotJoints"
...
class RobotSchemaRule(RuleInterface):
    ...
    def process_rule(self) -> str | None:
        ...
        with Usd.EditContext(self.source_stage, edit_layer):
            if not _HAS_ISAAC_SCHEMA:
                # Kit-free fallback: author the same USD the RC.007 validator
                # checks (IsaacRobotAPI applied + robotLinks/robotJoints
                # relationships) directly, without ``usd.schema.isaac``.
                root_link, root_joint = self._author_robot_schema_kitfree(self.source_stage, robot_prim)
                self.log_operation("Applied RobotAPI via Kit-free fallback (usd.schema.isaac unavailable)")
            else:
    ...
    def _author_robot_schema_kitfree(self, stage: Usd.Stage, robot_prim: Usd.Prim):
        """Author IsaacRobotAPI and robot relationships without ``usd.schema.isaac``.

        Adds the ``IsaacRobotAPI`` applied schema to ``robot_prim`` and authors
        the ``isaac:physics:robotLinks`` / ``isaac:physics:robotJoints``
        relationships from the descendant PhysX rigid bodies and joints. This
        satisfies the SimReady RC.007 checks in Kit-free runtimes where
        ``usd.schema.isaac`` is unavailable. ``isaac:robotType`` is intentionally
        not authored (RC.008/RC.009 remain a separate, deferred concern).

        Args:
            stage: The working stage being edited.
            robot_prim: The robot default prim receiving the schema.

        Returns:
            Tuple of (root link prim or None, root joint prim or None) for logging.
        """
        link_paths: list[Sdf.Path] = []
        joint_paths: list[Sdf.Path] = []
        articulation_root: Usd.Prim | None = None
        for descendant in Usd.PrimRange(robot_prim):
            if descendant.HasAPI(UsdPhysics.RigidBodyAPI):
                link_paths.append(descendant.GetPath())
            if descendant.IsA(UsdPhysics.Joint):
                joint_paths.append(descendant.GetPath())
            if articulation_root is None and descendant.HasAPI(UsdPhysics.ArticulationRootAPI):
                articulation_root = descendant

        # Author the IsaacRobotAPI applied schema directly on the edit-target
        # layer. ``Usd.Prim.AddAppliedSchema`` no-ops when the schema type is not
        # registered, which is exactly the Kit-free case, so edit the apiSchemas
        # list op through Sdf instead. Prepend semantics compose with any weaker
        # layers, and the downstream "Make API Schemas Non-Explicit" rule keeps
        # the list op non-explicit.
        self._prepend_applied_api_schema(robot_prim, _ROBOT_API_SCHEMA_NAME)

        links_rel = robot_prim.CreateRelationship(_ROBOT_LINKS_REL_NAME, custom=False)
        links_rel.SetTargets(link_paths)
        joints_rel = robot_prim.CreateRelationship(_ROBOT_JOINTS_REL_NAME, custom=False)
        joints_rel.SetTargets(joint_paths)

        self.log_operation(
            f"Kit-free robot schema: {len(link_paths)} link(s), {len(joint_paths)} joint(s)"
            + (f", articulation root {articulation_root.GetPath()}" if articulation_root else "")
        )

        root_link = stage.GetPrimAtPath(link_paths[0]) if link_paths else None
        root_joint = stage.GetPrimAtPath(joint_paths[0]) if joint_paths else None
        return root_link, root_joint

    @staticmethod
    def _prepend_applied_api_schema(prim: Usd.Prim, schema_name: str) -> None:
        """Prepend ``schema_name`` to ``prim``'s ``apiSchemas`` on the edit target.

        Authors the ``apiSchemas`` list op through Sdf so an unregistered applied
        API schema (for example ``IsaacRobotAPI`` in a Kit-free runtime) is still
        recorded. No-ops if the schema is already present in this layer's op.

        Args:
            prim: The prim whose ``apiSchemas`` list op is edited.
            schema_name: Applied API schema token to prepend.
        """
        layer = prim.GetStage().GetEditTarget().GetLayer()
        prim_spec = Sdf.CreatePrimInLayer(layer, prim.GetPath())
        list_op = prim_spec.GetInfo("apiSchemas")
        if schema_name in list_op.GetAddedOrExplicitItems():
            return
        if list_op.isExplicit:
            list_op.explicitItems = list(list_op.explicitItems) + [schema_name]
        else:
            list_op.prependedItems = list(list_op.prependedItems) + [schema_name]
