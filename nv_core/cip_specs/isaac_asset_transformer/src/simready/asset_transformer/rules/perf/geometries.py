...
class GeometriesRoutingRule(RuleInterface):
    ...
    @staticmethod
    def _author_local_physics_binding(prim_spec: Sdf.PrimSpec, targets: list[Sdf.Path]) -> None:
        """Author a physics-material binding where external target paths are valid."""
        if not targets:
            return
        relationship = prim_spec.relationships.get("material:binding:physics")
        if relationship is None:
            relationship = Sdf.RelationshipSpec(prim_spec, "material:binding:physics")
        relationship.targetPathList.ClearEdits()
        relationship.targetPathList.explicitItems = targets

    @staticmethod
    def _remove_instance_physics_bindings(layer: Sdf.Layer) -> None:
        """Remove bindings that cannot target materials outside a reference scope."""

        def visit(prim_spec: Sdf.PrimSpec) -> None:
            if "material:binding:physics" in prim_spec.properties:
                del prim_spec.properties["material:binding:physics"]
    ...
    def _update_source_stage_references(
        ...
        for instance_key, instance_entry in instance_by_key.items():
            ...
            for source in instance_entry.sources:
                ...
                physics_binding = source_prim.GetRelationship("material:binding:physics")
                physics_targets = list(physics_binding.GetTargets()) if physics_binding else []
                ...
                if should_merge:
                    ...
                    self._author_local_physics_binding(parent_prim_spec, physics_targets)
                    ...
                    self._author_local_physics_binding(prim_spec, physics_targets)

                    updated_count += 1

        self._remove_instance_physics_bindings(instances_layer)
