...
"""FET001 Backface culling / winding validation -- combined XZ rotation (VG.029).

WHAT: Verifies face winding order using combined X+Z rotation to expose
      diagonal-facing surfaces that pure X or Z rotation keeps edge-on.

HOW:  Same alternating capture pattern as culling_x/culling_z but uses
      combined rotation: X at 70% amplitude, Z at 100%. This sweeps the
      asset through angles that neither single-axis rotation covers.
      ...
      3. For each step, captures paired frames:
      ...
      4. Uses object-masked per-pixel comparison on a white background.

WHY:  Some winding defects only appear on faces oriented at diagonal angles
      (e.g., 45-degree chamfers, beveled edges). Pure X and Z rotation keep
      these faces edge-on. Combined XZ rotation exposes them.
      Together culling_x + culling_z + culling_xz cover all face
      orientations.
...
@test(
    ...
    name="culling_xz",
    description=(
        "Rotates the asset around a combined X+Z axis (a tilted spin) under "
        "dome lighting with a diagnostic orange matte material; verifies the "
        "orange color is continuously visible. Catches winding-order errors "
        "that present only at oblique view angles, which the pure-X and "
        "pure-Z variants might miss."
    ),
    expected_video=(
        "The asset rotating around a tilted axis combining pitch and yaw. "
        "Orange diagnostic surface fills the silhouette at every frame. "
        "Holes in the silhouette indicate inverted winding only seen at "
        "oblique angles."
    ),
    version="2.0.0",
...
async def test_culling_xz(ctx):
    """Backface culling / winding validation (combined XZ-axis rotation)."""
    ...
    ctx.step("Capturing culling pairs (XZ-axis rotation)")
    frames_off = []
    frames_on = []
    ...
    for i in range(steps):
        ...
        await ctx.settle()
        frames_off.append(await ctx.capture_frame(label="culling_xz_off"))

        ctx.scene.set_double_sided(False)
        await ctx.settle()
        frames_on.append(await ctx.capture_frame(label="culling_xz_on"))
    ...
    if not passed:
        ctx.encode_video(
            ...
            fps=1,
            delete_frames=False,
            label="culling_xz_worst_pair",
            role="worst",
        )

    # --- Videos (delete_frames=True frees disk after encoding) ---
    ctx.encode_video(frames_off, fps=2, label="culling_xz_culling_off", role="summary")
    ctx.encode_video(frames_on, fps=2, label="culling_xz_culling_on", role="summary")

    # --- Metrics ---
    ctx.add_metric("culling_xz_max_diff", round(max_diff, 6))
    ctx.add_metric("culling_xz_mean_diff", round(mean_diff, 6))
    ctx.add_metric("culling_xz_worst_angle", worst_idx * angle_per_step)
    ctx.add_metric("culling_xz_passed", 1 if passed else 0)

    # --- Pass/Fail ---
    if not passed:
        ctx.fail(
            "XZ-axis culling check failed at angle %.0f deg: "
            "%.1f%% object pixel diff (threshold %.0f%%).\n"
            "Fix: Check mesh face winding for diagonal-facing surfaces. "
            "Combined XZ rotation exposes faces that pure X or Z rotation "
            "keeps edge-on. In your DCC tool, ensure consistent "
            "counter-clockwise winding and recalculate normals outward."
            % (worst_idx * angle_per_step, max_diff * 100, threshold * 100),
        )
        return

    ctx.log("XZ-axis culling PASSED (max object pixel diff: %.1f%%)" % (max_diff * 100))
