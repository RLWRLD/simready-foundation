...
The `Prop-Robotics-Physx` profile includes the following feature sets (see `prop_robotics_physx.toml` and the [feature dependency graph](../features/feature-dependency-graph)). Each feature's requirements and dependencies are defined in the feature specifications.
...
variant set defaults to `Disabled`, so the base asset behavior is unchanged
...
- `nv_core/tiers/simready_foundation_tier_core/simready/foundation/tier_core/profiles/prop_robotics_physx.toml`
