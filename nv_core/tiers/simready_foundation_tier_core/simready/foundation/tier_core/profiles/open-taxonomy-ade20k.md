# Open-Taxonomy-ADE20K Profile

This document describes how to author a USD asset that conforms to the `Open-Taxonomy-ADE20K` profile.

`Open-Taxonomy-ADE20K` is an **example profile** that demonstrates labeling assets with the **ADE20K** taxonomy
instead of NVIDIA's Wikidata Q-codes. It pairs the Core feature with one closed-vocabulary taxonomy
feature (ADE.001). See [Adding a custom taxonomy](../guides/adding_a_custom_taxonomy.md) to build your own.

## Profile definition

The `Open-Taxonomy-ADE20K` profile includes the following feature set (see `open_taxonomy_profiles.toml`).
Each feature's requirements and dependencies are defined in the feature specifications.

```toml
[Open-Taxonomy-ADE20K]
"0.1.0" = {features = [
    {"FET_000_STANDARD" = {version = "0.1.0"}}, # Core
    {"FET_042_STANDARD" = {version = "0.1.0"}}, # ADE20K Labels
]}
```
...
### Semantic labels (ADE20K)

- Apply `SemanticsLabelsAPI:ade20k` to prims you want labeled with this taxonomy.
- Author `token[] semantics:labels:ade20k` using **ADE20K class names** (ADE.001).
...
{
    token[] semantics:labels:ade20k = ["wall", "building"]
}
```
...
- `docs/features/FET_042_STANDARD.md`
