...
`Open-Taxonomy-SUNRGBD` is an **example profile** that demonstrates labeling assets with the **SUN RGB-D** taxonomy
instead of NVIDIA's Wikidata Q-codes. It pairs the Core feature with one closed-vocabulary taxonomy
feature (SUN.001). See [Adding a custom taxonomy](../guides/adding_a_custom_taxonomy.md) to build your own.

## Profile definition

The `Open-Taxonomy-SUNRGBD` profile includes the following feature set (see `open_taxonomy_profiles.toml`).
