# Open-Taxonomy-ImageNet1K Profile

This document describes how to author a USD asset that conforms to the `Open-Taxonomy-ImageNet1K` profile.

`Open-Taxonomy-ImageNet1K` is an **example profile** that demonstrates labeling assets with the **ImageNet-1K** taxonomy
instead of NVIDIA's Wikidata Q-codes. It pairs the Core feature with one closed-vocabulary taxonomy
feature (IN1K.001). See [Adding a custom taxonomy](../guides/adding_a_custom_taxonomy.md) to build your own.

## Profile definition

The `Open-Taxonomy-ImageNet1K` profile includes the following feature set (see `open_taxonomy_profiles.toml`).
Each feature's requirements and dependencies are defined in the feature specifications.

```toml
[Open-Taxonomy-ImageNet1K]
"0.1.0" = {features = [
    {"FET_000_STANDARD" = {version = "0.1.0"}}, # Core
    {"FET_045_STANDARD" = {version = "0.1.0"}}, # ImageNet-1K Labels
]}
```
...
### Semantic labels (ImageNet-1K)

- Apply `SemanticsLabelsAPI:imagenet_1k` to prims you want labeled with this taxonomy.
- Author `token[] semantics:labels:imagenet_1k` using **ImageNet-1K class names** (IN1K.001).
...
{
    token[] semantics:labels:imagenet_1k = ["tench", "goldfish"]
}
```
...
- `docs/features/FET_045_STANDARD.md`
