...
When an asset authors the `PhysX` variant set (see [physx-variant-set](physx-variant-set) `RV.001`), the root layer must carry a matching descriptor so tooling can discover and activate the variant without traversing composition. The descriptor lives at:
...
- `variantSetName` (string): the variant set name, `PhysX`.
- `activateOption` (string): the option that turns PhysX on, i.e. `Enabled`.
...
- Under `Variants.Physics`, add a `PhysX` dictionary.
- Set `prim` to the default prim path, `variantSetName` to `PhysX`, and `activateOption` to `Enabled`.
