...
* **Revolute (and other non-prismatic) mimics:** enforce drive stiffness == 0.0 and drive damping == 0.0
* **Prismatic mimics:** author a non-zero drive stiffness within the normal reasonable range (PhysX does not enforce linear mimic, so the follower drive must remain active; damping alone is not enough)
