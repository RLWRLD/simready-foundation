...
# SimReady Conform FET_006_STANDARD
...
2. Load the selected `FET_006_STANDARD` manifest and the feature markdown before editing.
...
5. Repair only the requirements listed by the selected `FET_006_STANDARD` manifest and its dependencies.
...
7. Summarize the selected `FET_006_STANDARD` version, changed files, validation evidence, and the first remaining blocker or next exact feature gate.

## Feature Guidance

- Repair only the requirements listed by the selected `FET_006_STANDARD` manifest and its dependencies.
...
| Field | Meaning |
|---|---|
| `feature` | Exact feature ID and version, for example `FET_006_STANDARD@0.1.0`. |
